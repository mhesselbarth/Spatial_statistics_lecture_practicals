...
```{r, message = FALSE}
  ...
  hemlock <- read_delim(file = 'Hemlock_Adults_vs_SmallSaps.txt', delim = ';')
...
```{r}
  Oest <- function(input, correction = 'Ripley', r = NULL, ...){
    p_fct <- pcf(input, correction = correction, 
                 r = r, divisor = 'd')
...
We are just interested in the saplings, therefore we subset them first. Then we calculate the O-ring statistic with simulation envelopes and _CSR_ as null model.
...
```{r, results = 'hide'}
  thomas_process <- kppm(X = unmark(saplings), clusters = "Thomas", statistic = "pcf")
...
```{r, echo = FALSE}
  paste0("Number of parents: ", round(thomas_process$clustpar[['kappa']] * area(saplings$window), 2))
  paste0("Cluster area: ", round((thomas_process$clustpar[['scale']] ^ 2) * pi, 2)) # Double check also with Programita!
```

# Practical 3b

## Exercise 1 
__Task:__ The data set 'Alive_Douglas-Fir_OG_N.txt' contains the location and stem diameters (diameter at breast height, DBH) for all live Douglas fir trees in the Old-growth North plot. Calculate the mark correlation function What do these results suggest?

First, we need to import the data, convert as `ppp` and have a look at the data. This time, the marks are continuous, i.e. the diameter at breast height in cm of each tree. Therefore, the `summary()` function automatically gives the mean, median and quantiles of the data. Also, the plot automatically scales the point according to their size. 

```{r, message = FALSE}
  setwd('C:/Users/Maximilian/ownCloud/03_Lehre/Spatial_Statistics/Practicals/Spatial_statistics_R')
  douglas_fir <- read_delim(file = 'Alive_Douglas-Fir_OG_N.txt', delim = ';')
...
```{r, results = 'hide'}
  envelopes_mark_correlation_alive <- envelope(Y = douglas_fir_ppp, fun = markcorr,
                                               r = seq(from = 0, to = 45, by = 0.5),
                                               nsim = 199, nrank = 5,
...
We import the data, classify it and convert it as a `ppp` object. We plot the point pattern of only adult trees and have a look at the summary.
...
```{r, echo = FALSE, fig.align = 'center'}
  plot(subset.ppp(hemlock_ppp, marks == 'adults', drop=T), main='Hemlock \nadults')
```
...
We calculate the O-ring statistic for the adult trees only, using `envelope()` and the null model of _CSR_. 
```{r, results = 'hide'}
...
__Task 2:__ Getzin et al. (2008) show that the study area is heterogeneous. We are going to account for this heterogeneity by fitting a heterogeneous Poisson processes.
...
```{r}
  lambda_xy <- density.ppp(x = adults)
  class(lambda_xy)
```
...
<!-- ```{r, results = 'hide'} -->
<!--   envelopes_adults_inhomo <- envelope(Y = adults, fun = pcfinhom, divisor = "d", -->
<!--                                       correction = "Ripley",  -->
<!--                                       r = seq(from = 0, to = 45, by = 0.5),  -->
<!--                                       nsim = 199, rank = 5) -->
<!-- ``` -->
...
<!--        legend = FALSE, main = 'Inhomogenous pair correlation function') -->
<!-- ``` -->
